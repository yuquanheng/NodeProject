...
var ostable =$('#xunjiantable').dataTable( {
		...
		"sAjaxSource": "http://127.0.0.1:3000/dianming/guard/queryArea",
...
$('#xunjiantable tbody ').on('click', 'button',function (event) 
{
   ...
   if(buttonID == "edit")
   {
   	 ...
   	 var user = {'id':aData[1],'phone':aData[3],'name':aData[2],'areagroup':aData[5]};
   	 window.localStorage.removeItem("GuardID");
   	 window.localStorage.setItem("GuardID",JSON.stringify(user));
     console.log("GuardID id "+window.localStorage.getItem("GuardID"));
     window.location.href = "postUpGuard.html"
   }
...
$("#addperson").click(function(){
    ...
    window.localStorage.removeItem("GuardID");
	//window.open("postGuard.html");
	window.location.href = "postNewGuard.html";
});
//删除操作
$('#deletebutton').on('click',function(){
	$.ajax({  
           type : "post",  //提交方式  
           url  : "http://127.0.0.1:3000/dianming/guard/delArea?time="+new Date().getTime(),//路径
...
function updatefor(){
           ...
           success : function(result) {//返回数据根据结果进行相应的处理  
              var datalength = result.length;
               ...
               {  
			     $('#area').append("<option value='"+result[i]['L_ID'] + "'>"+result[i]['L_Name']+"</option>");
               }
