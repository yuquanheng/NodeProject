...
var myconnect = require('../sql/dbhelp.js');
...
var n = childProcess.fork('./routes/xunjianProject/redisprocessforxunjian.js'); 
//用户名，密码和数据库服务器,数据库   
		...
		else
		{
		  if(timestart != "")
		   ...
		   {   
			   ...
			   if(timeend !="")
			   {
				   sql = "select * from dbo.Elec_InspLineReport ReportDate = '"+timeend+"'";
				   if(gongsi!="")
		           {  
			         sql = sql+" and OrgName like '%"+gongsi+"%'";
		           }
