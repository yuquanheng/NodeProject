...
function getparaoso(eid)
{
	var deferred = Q.defer();
	sql = "select OrgName from dbo.GIS_MileageReport where E_ID = "+eid;
	var connectionkey = new Connection(config);
	myconnect.querydata(sql,connectionkey,function(err,pdata){
				...
				else
				{
					if(pdata)
					{ 
						console.log("i get the os nae  ");
						var plength = pdata.length;
						var parr = [];
						var puarr = "";
						for(var j=0;j<plength;j++)
						{
							parr.push(pdata[j]['OrgName']);
						}
						Unique.arr = parr;
						puarr = Unique.StringGroup();
						deferred.resolve(puarr);
					}
					else
					{
						console.log("没有数据");
...
function getparaos(ename)
{
	var deferred = Q.defer();
	sql = "select E_ID from dbo.SYS_EnterpriseInfo where E_Name = '"+ename+"'";
	...
	else
	{
		if(data.length > 0)
		{
			var pudongeid = data[0]['E_ID'];
			console.log("get emy ID "+pudongeid);
			deferred.resolve(pudongeid);
		}
		else
		{   
			console.log("没有数据");
...
function joinsql(args){
  ...
  if(sql)
  {
	  if(timestart)
	  {
		  ...
		  {
			sql = sql+" and convert(varchar(24),ReportDate,121) = '"+timestart+"'";
		  } 
	  }
	  else
	  {
		  if(timeend)
		  {
			sql = sql+" and convert(varchar(24),ReportDate,121) = '"+timeend+"'";  
		  }
	  }
  }
  else
  {
	  if(timestart)
	  {
		  ...
		  {
			sql ="select * from dbo.GIS_MileageReport where convert(varchar(24),ReportDate,121) = '"+timestart+"'";
		  } 
	  }
	  else
	  {
		  if(timeend)
		  {
			sql ="select * from dbo.GIS_MileageReport where convert(varchar(24),ReportDate,121) = '"+timeend+"'";
		  }
	  }
